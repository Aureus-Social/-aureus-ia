...
function Reviews() {
  ...
  useEffect(() => { const iv = setInterval(() => setCur(c => (c + 1) % reviews.length), 5000); return () => clearInterval(iv); }, [reviews.length]);
  const r = reviews[cur];
  ...
  return (
    <div className="rv">
      <a href="https://maps.app.goo.gl/VMUHTHixnwayVXV96" target="_blank" rel="noopener noreferrer" style={{ display:"inline-flex",alignItems:"center",gap:10,background:"rgba(255,255,255,.07)",border:"1px solid rgba(201,168,76,.3)",borderRadius:12,padding:"10px 20px",marginBottom:28,textDecoration:"none",transition:"all .3s" }} onMouseOver={e=>(e.currentTarget.style.background="rgba(201,168,76,.12)")} onMouseOut={e=>(e.currentTarget.style.background="rgba(255,255,255,.07)")}>
        ...
        <span style={{ color:"#C9A84C",fontWeight:700,fontSize:15 }}>{rating.toFixed(1)}</span>
        <span style={{ color:"#C9A84C",fontSize:13 }}>★★★★★</span>
        <span style={{ color:"rgba(255,255,255,.6)",fontSize:13 }}>Google Reviews ({totalReviews})</span>
      </a>
      <div className="rvq">&ldquo;</div>
      <p className="rvt">{r.text}</p>
      <div className="rvs">{stars}</div>
      <div className="rvn">{r.author}</div>
      <div className="rvr">Google Review{r.time ? ` · ${r.time}` : ""}</div>
      <div className="rvd">{reviews.map((_, i) => (<button key={i} className={`rvdt ${i === cur ? "act" : ""}`} onClick={() => setCur(i)} />))}</div>
